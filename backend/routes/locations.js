...
router.get('/', protect, async (req, res) => {
  try {
    ...
    const formatted = locations.map(loc => ({
      ...
      center_lat: loc.centerLat != null ? parseFloat(loc.centerLat) : null,
      center_lng: loc.centerLng != null ? parseFloat(loc.centerLng) : null,
      radius_meters: loc.radiusMeters != null ? parseInt(loc.radiusMeters, 10) : null,
      boundaries: loc.boundaries || null, // Polygon boundaries as [[lng, lat], ...]
...
router.post('/', protect, authorize('ceo', 'super_admin', 'general_manager', 'admin_assistant'), async (req, res) => {
  try {
...
// @access  Private/Admin
router.put('/:id', protect, authorize('ceo', 'super_admin', 'general_manager', 'admin_assistant'), async (req, res) => {
  try {
