...
router.get('/:id', protect, async (req, res) => {
  try {
    ...
    // Try to find by id first, then by deptId
    if (!isNaN(id)) {
      department = await Department.findOne({ id: id, isActive: true });
      if (!department) {
        department = await Department.findOne({ deptId: id, isActive: true });
      }
    } else {
      department = await Department.findOne({ id: req.params.id, isActive: true });
    }
...
router.put('/:id', protect, authorize('ceo', 'super_admin'), async (req, res) => {
  try {
    ...
    // Try to find by id first, then by deptId
    if (!isNaN(id)) {
      department = await Department.findOne({ id: id });
      if (!department) {
        department = await Department.findOne({ deptId: id });
      }
    } else {
      department = await Department.findOne({ id: req.params.id });
    }
    ...
    department = await Department.findByIdAndUpdate(department.id, updateData);
...
router.delete('/:id', protect, authorize('ceo', 'super_admin'), async (req, res) => {
  try {
    ...
    await Department.findByIdAndDelete(department.id);
