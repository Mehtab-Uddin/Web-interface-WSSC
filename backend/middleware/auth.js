  ...
  try {
    ...
    try {
      ...
      return res.status(401).json({
        success: false,
        error: 'Not authorized to access this route',
        message: err.name === 'JsonWebTokenError' ? 'Invalid token' : 'Token verification failed'
      });
    }
  } catch (error) {
...
// Check if user can manage other users (create/update)
exports.canManageUsers = (role) => {
  const normalized = exports.normalizeRole(role);
  return ['ceo', 'super_admin', 'general_manager', 'manager', 'admin_assistant'].includes(normalized);
};
